...
- Background correction defaults to `Off` on the main screen. When it is turned `On`, a final preview window opens before the PNG is saved. In that window, `Automatic background correction` estimates and subtracts a smooth per-band background before RGB composition. `Valid field mask` applies a soft circular mask to reduce artifacts outside the useful optical field and lets you tune the mask radius, softness and outside correction strength. Outside correction `0` leaves the image unmasked, while `1` fully applies the mask outside the useful field.
